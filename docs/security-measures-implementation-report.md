...
Configure GitHub branch protection from `Settings` -> `Rules` -> `Rulesets` -> `New branch ruleset`.

The ruleset should look like the provided GitHub screenshot, with these completed values:
...
Required checks to add after workflows have run at least once:
...
Do not leave the ruleset in the screenshot's incomplete state:
