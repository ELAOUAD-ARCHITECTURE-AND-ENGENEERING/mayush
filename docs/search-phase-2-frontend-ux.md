...
| `php artisan test tests/Feature/Frontend/SearchUxContractTest.php` | Passed: 2 tests, 14 assertions |
...
The `npm run build:storefront` wrapper also remains unavailable because the installed global npm CLI is missing `npm-cli.js`; the direct repository build and manifest validation did pass.
