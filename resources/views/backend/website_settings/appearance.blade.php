    ...
    <div class="col-lg-8 mx-auto">
        ...
        <div class="card">
            <div class="card-header">
                <h1 class="mb-0 h6">{{translate('System Settings')}}</h1>
            </div>
            <div class="card-body">
        ...
        <div class="card">
            <div class="card-header">
                <h6 class="fw-600 mb-0">{{ translate('Image Optimization') }}</h6>
            </div>
            <div class="card-body">
                <form action="{{ route('business_settings.update') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="form-group row">
                        <label class="col-md-3 col-from-label">{{translate('Disable Image Optimization')}}</label>
                        <div class="col-md-8">
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input type="hidden" name="types[]" value="disable_image_optimization">
                                <input type="checkbox" name="disable_image_optimization" @if( get_setting('disable_image_optimization')=='on' || get_setting('disable_image_optimization')==1 ) checked @endif>
                                <span></span>
                            </label>
                            <small class="text-muted d-block mt-1">{{ translate('If enabled, original images will be saved without resizing or compression. Not recommended.') }}</small>
                        </div>
                    </div>
                    
                    <!-- Uploaded Image Format -->
                    <div class="form-group row">
                        <label class="col-md-3 col-from-label">{{ translate('Output Image Format') }}</label>
                        <div class="col-md-8">
                            <input type="hidden" name="types[]" value="uploaded_image_format">
                            <select name="uploaded_image_format" class="form-control aiz-selectpicker">
                                <option value="default" @if (get_setting('uploaded_image_format')=="default" || get_setting('uploaded_image_format')=="") selected @endif>{{ translate('Original (No format change)') }}</option>
                                <option value="webp" @if (get_setting('uploaded_image_format')=="webp" ) selected @endif>{{ translate('WebP (Recommended for best performance)') }}</option>
                                <option value="jpg" @if (get_setting('uploaded_image_format')=="jpg" ) selected @endif>{{ translate('JPEG') }}</option>
                                <option value="png" @if (get_setting('uploaded_image_format')=="png" ) selected @endif>{{ translate('PNG') }}</option>
                            </select>
                            <small class="text-muted">{{ translate('SVG and GIF images bypass conversion regardless of this setting.') }}</small>
                        </div>
                    </div>

                    <!-- Image Quality -->
                    <div class="form-group row">
                        <label class="col-md-3 col-from-label">{{ translate('Compression Quality') }}</label>
                        <div class="col-md-8">
                            <input type="hidden" name="types[]" value="image_quality">
                            <div class="d-flex align-items-center">
                                <input type="range" class="custom-range flex-grow-1" min="40" max="100" step="5" name="image_quality" id="image_quality_slider" value="{{ get_setting('image_quality', 80) }}" oninput="document.getElementById('image_quality_output').value = this.value">
                                <output id="image_quality_output" class="ml-3 fw-600 w-30px text-right">{{ get_setting('image_quality', 80) }}</output>
                            </div>
                            <small class="text-muted">{{ translate('Recommended range: 75-85. Lower values save space but reduce visual quality.') }}</small>
                        </div>
                    </div>

                    <!-- Max Dimension (Width/Height) -->
                    <div class="form-group row">
                        <label class="col-md-3 col-from-label">{{ translate('Maximum Dimensions (px)') }}</label>
                        <div class="col-md-8">
                            <input type="hidden" name="types[]" value="image_max_width">
                            <select name="image_max_width" class="form-control aiz-selectpicker">
                                <option value="800" @if (get_setting('image_max_width')=="800" ) selected @endif>800px</option>
                                <option value="1000" @if (get_setting('image_max_width')=="1000" ) selected @endif>1000px</option>
                                <option value="1200" @if (get_setting('image_max_width')=="1200" ) selected @endif>1200px</option>
                                <option value="1500" @if (get_setting('image_max_width', "1500")=="1500" ) selected @endif>1500px</option>
                                <option value="2000" @if (get_setting('image_max_width')=="2000" ) selected @endif>2000px</option>
                                <option value="3000" @if (get_setting('image_max_width')=="3000" ) selected @endif>3000px</option>
                            </select>
                            <small class="text-muted">{{ translate('Images larger than this will be proportionally scaled down to fit.') }}</small>
                        </div>
                    </div>
