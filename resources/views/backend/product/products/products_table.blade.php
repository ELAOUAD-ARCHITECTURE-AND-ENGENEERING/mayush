    ...
    <table class="table mb-0" id="aiz-data-table">
         <thead>
            <tr>
                ...
                <th class="hide-xl text-uppercase fs-12 fw-700 text-secondary">{{ translate('Stock') }}</th>
        ...
        <tbody>
            ...
            <tr class="data-row {{ ($show_unpublished ?? false) && $is_unpublished ? 'unpublished-row' : '' }}">
                ...
                <td class="hide-xl align-middle" data-label="{{ translate('Stock') }}">
                    @if(!$product->draft && !$product->digital)
                        @php
                            $total_stock = 0;
                            if ($product->variant_product) {
                                ...
                                $total_stock = $product->stocks->first()->qty ?? 0;
                            }
                        @endphp
                        <span class="fs-14 fw-700 
                            @if($total_stock > 10) text-success
                            @elseif($total_stock > 0) text-warning
                            @else text-danger
                            @endif">
                            {{ $total_stock }}
                        </span>
                    @else
                        <span class="fs-14 fw-400 text-secondary">{{ translate('N/A') }}</span>
                    @endif
                </td>
                ...
                @if (get_setting('product_approve_by_admin') == 1 && $type == 'seller')
                <td class="hide-xxl align-middle" data-label="{{ translate('Approved') }}">
                     @if (!$product->draft)
                    <label class="aiz-switch aiz-switch-blue mb-0">
                ...
                <td class="hide-xxl align-middle" data-label="{{ translate('TodaysDeal') }}">
                    @if (!$product->draft)
                    <label class="aiz-switch aiz-switch-blue mb-0">
