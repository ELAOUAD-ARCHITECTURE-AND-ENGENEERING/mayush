    ...
    <tbody>
        ...
        @foreach ($combinations as $key => $combination)
            @php
                ...
                $normalizedVariant = strtolower(preg_replace('/\s+/', '', trim($str)));
                if (!isset($stockIndexMap[$normalizedVariant])) {
                    $stockIndexMap[$normalizedVariant] = 0;
                } else {
                    $stockIndexMap[$normalizedVariant]++;
                }
                $occurrenceIndex = $stockIndexMap[$normalizedVariant];

                $variantStocks = $product->stocks->where('variant', $str)->values();
                $stock = $variantStocks->get($occurrenceIndex);
            @endphp
            @if(strlen($str) > 0)
            <tr class="variant">
                <td>
                ...
                <td>
                    <input type="number" lang="en" name="price_{{ $str }}@if($dimensionSkuRowsEnabled)[]@endif" value="@php
                ...
                <td>
                    <input type="text" name="sku_{{ $str }}@if($dimensionSkuRowsEnabled)[]@endif" value="@php
                            if($stock != null) {
                                echo $stock->sku;
                            }
                            else {
                ...
                <td>
                    <input type="number" lang="en" name="qty_{{ $str }}@if($dimensionSkuRowsEnabled)[]@endif" value="@php
                            if($stock != null){
                                echo $stock->qty;
                            }
                            else{
                ...
                <td>
                    <div class="row gutters-5">
                        <div class="col">
                            <input type="number" lang="en" name="length_{{ $str }}@if($dimensionSkuRowsEnabled)[]@endif" value="{{ $stock->length ?? 0 }}" min="0" step="0.01" class="form-control" placeholder="{{ translate('L') }}">
                        </div>
                        <div class="col">
                            <input type="number" lang="en" name="width_{{ $str }}@if($dimensionSkuRowsEnabled)[]@endif" value="{{ $stock->width ?? 0 }}" min="0" step="0.01" class="form-control" placeholder="{{ translate('W') }}">
                        </div>
                        <div class="col">
                            <input type="number" lang="en" name="height_{{ $str }}@if($dimensionSkuRowsEnabled)[]@endif" value="{{ $stock->height ?? 0 }}" min="0" step="0.01" class="form-control" placeholder="{{ translate('H') }}">
                        </div>
                    </div>
                </td>
                <td>
                    <select name="unit_{{ $str }}@if($dimensionSkuRowsEnabled)[]@endif" class="form-control aiz-selectpicker">
                        <option value="cm" @selected(($stock->dimension_unit ?? 'cm') == 'cm')>cm</option>
                        <option value="inch" @selected(($stock->dimension_unit ?? 'cm') == 'inch')>inch</option>
                    </select>
                </td>
                <td>
                    <div class="input-group" data-toggle="aizuploader" data-type="image">
                        ...
                        <input type="hidden" name="img_{{ $str }}@if($dimensionSkuRowsEnabled)[]@endif" class="selected-files" value="@php
                                if($stock != null){
                                    echo $stock->image;
                                }
                                else{
