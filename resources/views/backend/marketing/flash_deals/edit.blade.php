            ...
            <div class="card-body p-0">
              <ul class="nav nav-tabs nav-fill language-bar">
                @foreach (get_all_active_language() as $key => $language)
                  <li class="nav-item">
                    <a class="nav-link text-reset @if ($language->code == $lang) active @endif py-3" href="{{ route('flash_deals.edit', ['flash_deal'=>$flash_deal->id, 'lang'=> $language->code] ) }}">
              ...
              <form class="p-4" action="{{ route('flash_deals.update', $flash_deal) }}" method="POST">
                @csrf
