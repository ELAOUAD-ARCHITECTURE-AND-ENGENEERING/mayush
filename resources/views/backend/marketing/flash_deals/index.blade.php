            ...
            <tbody>
                @foreach($flash_deals as $key => $flash_deal)
                    <tr>
						...
						<td class="text-right">
                            @can('edit_flash_deal')
                                <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{route('flash_deals.edit', ['flash_deal'=>$flash_deal->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" title="{{ translate('Edit') }}">
                                    <i class="las la-edit"></i>
                                </a>
                            @endcan
                            @can('delete_flash_deal')
                                <form action="{{ route('flash_deals.destroy', $flash_deal->id) }}" method="POST" class="d-inline-block">
                                    @csrf
                                    @method('DELETE')
