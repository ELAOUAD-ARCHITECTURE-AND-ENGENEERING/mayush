						...
						<div class="aiz-file-box">
							<div class="dropdown-file" >
								...
								<div class="dropdown-menu dropdown-menu-right">
									...
									<a href="{{ route('download_attachment', $file->id) }}" class="dropdown-item">
...
@section('script')
	<script type="text/javascript">
		...
		function bulk_delete() {
            ...
            $('#bulk-delete-spinner').removeClass('d-none');
            $('.bulk-delete-msg').addClass('d-none');
            $('#proceed-btn').addClass('disabled').attr('onclick', '');
            ...
            $.ajax({
                ...
                success: function (response) {
                    if(response.status == 1) {
                        AIZ.plugins.notify('success', response.message + ' (' + response.success_count + ' ' + '{{ translate('success') }}' + ', ' + response.fail_count + ' ' + '{{ translate('failed') }}' + ')');
                        setTimeout(function(){
                            location.reload();
                        }, 1000);
                    }
					else{
                        $('#bulk-delete-spinner').addClass('d-none');
                        $('.bulk-delete-msg').removeClass('d-none');
                        $('#proceed-btn').removeClass('disabled').attr('onclick', 'bulk_delete()');
