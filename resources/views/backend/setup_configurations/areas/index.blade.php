                        ...
                        <tbody>
                            @foreach($areas as $key => $area)
                                <tr>
                                    ...
                                    <td class="text-right">
                                        <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{ route('areas.edit', ['area'=>$area->id, 'lang'=>env('DEFAULT_LANGUAGE')]) }}" title="{{ translate('Edit') }}">
...
@section('script')
    <script type="text/javascript">
        ...
        function update_status(el){
            ...
            $.post('{{ (Route::has('areas.status') ? route('areas.status') : '#') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
       ...
       $('#sort_state').on('change', function () {
        ...
        $('#sort_country').on('change', function () {
            ...
            if (countryId) {
                $.ajax({
                    url: '{{ (Route::has('get-cities-by-country') ? route('get-cities-by-country') : '#') }}',
                    type: 'GET',
