        ...
        <div class="d-flex justify-content-around align-items-center align-items-stretch">
            ...
            @can('view_notifications')
                @if (config('notifications_v2.enabled'))
                    @include('partials.notification-center-trigger', ['variant' => 'admin'])
                @else
                    {{-- Preserve the existing admin inbox while the additive v2 rollout is disabled. --}}
                    @include('partials.legacy-notification-trigger', ['variant' => 'admin'])
                @endif
            @endcan
