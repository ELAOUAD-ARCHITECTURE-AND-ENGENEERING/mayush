...
<div class="td-grid-sr" style="margin-bottom:16px">
    <div class="td-card">
        <div class="td-card-title">Revenue Analytics & Forecast</div>
        <div class="td-card-sub">Historical sales with 7-day predictive modeling</div>
        <div id="chart-forecast" style="min-height:300px"></div>
    </div>
    <div class="td-card">
        <div class="td-card-title">Traffic Composition</div>
        <div class="td-card-sub">Session distribution by source</div>
...
<div class="td-grid-3" style="margin-bottom:16px">
    <div class="td-card">
        <div class="td-card-title">Hourly Traffic</div>
        <div class="td-card-sub">Visits per hour today</div>
        <div id="chart-hourly" style="min-height:180px"></div>
    </div>
    <div class="td-card">
        <div class="td-card-title">Conversion Funnel</div>
        <div class="td-card-sub">User journey breakdown</div>
        @php $stages = [
    ...
    <div class="td-card" style="text-align:center">
        <div class="td-card-title">Growth Velocity</div>
        <div class="td-card-sub">Month-over-month trajectory</div>
        ...
        <div style="font-size:11px;color:#10b981;font-weight:700;letter-spacing:.08em">{{ ($fc['growth_rate']??0)>=18?'OUTPERFORMING':'GROWTH MOMENTUM' }}</div>
        <div class="td-grid-2" style="margin-top:12px;gap:8px">
            ...
            <div class="td-micro-kpi"><div class="label">{{ $m['l'] }}</div><div class="val" style="color:{{ $m['c'] }}">{{ number_format($m['v']) }}</div></div>
            @endforeach
        </div>
    </div>
</div>
...
<div class="td-grid-sl" style="margin-bottom:16px">
    <div class="td-card">
        <div class="td-card-title">Top Vendors</div>
        <div class="td-card-sub">By revenue this period</div>
        <table class="td-table">
            ...
            @empty
            <tr><td colspan="4" style="text-align:center;color:#94a3b8;padding:20px">No vendor data this period</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
    <div>
        <div class="td-card">
            <div class="td-card-title">System Health</div>
            <div class="td-card-sub">API & service status</div>
            <table class="td-table">
                <thead><tr><th>Service</th><th>Uptime</th><th>Latency</th><th>Status</th></tr></thead>
                <tbody>
                ...
                <tr>
                    ...
                    <td><span class="td-status-dot" style="background:{{ $s['status']==='ok'?'#10b981':'#f59e0b' }}"></span>{{ $s['status']==='ok'?'OK':'Warn' }}</td>
                </tr>
                @empty
                <tr><td colspan="4" style="text-align:center;color:#94a3b8;padding:15px">Monitoring initializing...</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
        <div class="td-card">
            <div class="td-card-title">Automated Insights</div>
            <div class="td-card-sub">Performance signals</div>
            ...
            <div class="td-insight td-insight-{{ $a['level']??'info' }}">
                ...
                <div><div style="font-size:13px;font-weight:700;color:#0f172a">{{ $a['title'] }}</div><div style="font-size:11px;color:#64748b;margin-top:2px">{{ $a['message'] }}</div></div>
            </div>
            @empty
            <div style="text-align:center;padding:20px;color:#94a3b8;font-size:12px">🧘 All systems nominal</div>
            @endforelse
        </div>
    </div>
</div>
