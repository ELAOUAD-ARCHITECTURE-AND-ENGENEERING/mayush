...
@section('panel_content')
    ...
    <form class="" action="{{route('customer_products.update', $product->id)}}" method="POST" enctype="multipart/form-data" id="choice_form">
        ...
        <div class="card rounded-0 shadow-none border">
            <div class="card-header border-bottom-0">
                <h5 class="mb-0 fs-18 fw-700 text-dark">{{translate('Promotion')}}</h5>
            </div>
            <div class="card-body">
                @php
                    $current_promotion = \App\Models\Promotion::where('product_id', $product->id)->whereIn('status', ['approved', 'awaiting_admin_review'])->first();
                @endphp
                
                @if($current_promotion)
                    <div class="alert alert-info">
                        {{ translate('This product is currently promoted.') }} <br>
                        <strong>{{ translate('Status') }}:</strong> {{ ucfirst(str_replace('_', ' ', $current_promotion->status)) }} <br>
                        <strong>{{ translate('Dates') }}:</strong> {{ $current_promotion->start_date }} - {{ $current_promotion->end_date }}
                    </div>
                @else
                    <div class="form-group row">
                        <label class="col-md-2 col-from-label">{{translate('Promote this product')}}</label>
                        <div class="col-md-10">
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input type="checkbox" name="promote_product" id="promote_product_toggle">
                                <span></span>
                            </label>
                            <small class="form-text text-muted">{{ translate('Enable to request promotion for this product. Requires 1 upload credit.') }}</small>
                        </div>
                    </div>

                    <div id="promotion_fields" style="display: none;">
                        <div class="form-group row">
                            <label class="col-md-2 col-from-label">{{translate('Promotion Tier')}}</label>
                            <div class="col-md-10">
                                <select class="form-control aiz-selectpicker" name="promotion_tier">
                        ...
                        <div class="form-group row">
                            <label class="col-md-2 col-from-label">{{translate('Start Date')}}</label>
                            <div class="col-md-10">
                                <input type="date" class="form-control rounded-0" name="promotion_start_date" min="{{ date('Y-m-d') }}">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label class="col-md-2 col-from-label">{{translate('End Date')}}</label>
                            <div class="col-md-10">
                                <input type="date" class="form-control rounded-0" name="promotion_end_date" min="{{ date('Y-m-d') }}">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label class="col-md-2 col-from-label">{{translate('Notes')}}</label>
                            <div class="col-md-10">
                                <textarea class="form-control rounded-0" name="promotion_notes" rows="3"></textarea>
                            </div>
                        </div>
                    </div>
                @endif
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#promote_product_toggle').change(function() {
            if(this.checked) {
