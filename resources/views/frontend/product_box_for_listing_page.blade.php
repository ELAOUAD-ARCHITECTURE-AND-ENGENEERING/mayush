...
<div class="aiz-card-box h-auto bg-white py-3 hov-scale-img">
    <div class="position-relative  img-fit overflow-hidden" >
        ...
        @if ($customLabels)
            ...
            @endforeach
        @endif
        @if (isset($is_ai_mode) && $is_ai_mode && isset($score))
            <span class="absolute-top-left ai-match-badge @if($score > 0.8) ai-match-high @endif"
                style="top:{{ 25 * $badgeIndex }}px; left: 8px; margin-top: 8px;">
                <i class="las la-magic"></i>
                <span class="fs-11 fw-700">{{ round($score * 100) }}% {{ translate('Vibe Match') }}</span>
            </span>
            @php $badgeIndex++; @endphp
        @endif
