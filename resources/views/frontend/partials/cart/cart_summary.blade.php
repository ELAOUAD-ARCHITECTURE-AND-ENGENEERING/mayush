<div class="z-3 sticky-top-lg">
    <div class="card rounded-0 border">
        ...
        <div class="card-body pt-2">
            ...
            @if ($proceed != 1)
                @php
                    $suggestions = \App\Services\CartEnrichmentService::getSuggestions(2);
                @endphp
                @if ($suggestions->count() > 0)
                    <div class="mt-4 pt-4 border-top">
                        <h4 class="fs-14 fw-700 mb-3 text-uppercase">{{ translate('Complete The Look') }}</h4>
                        <div class="suggestions-list">
                            @foreach ($suggestions as $suggestion)
                                <div class="d-flex mb-3 align-items-center suggestion-item p-2 rounded has-transition hov-bg-soft-light" style="border: 1px solid #f1f1f1;">
                                    <div class="size-60px flex-shrink-0">
                                        <img src="{{ static_asset('assets/img/placeholder.jpg') }}" data-src="{{ uploaded_asset($suggestion->thumbnail_img) }}" 
                                             class="img-fit h-100 lazyload rounded" alt="{{ $suggestion->name }}">
                                    </div>
                                    <div class="ml-3 flex-grow-1 minw-0">
                                        <h5 class="fs-13 fw-600 mb-1 text-truncate-2">{{ $suggestion->getTranslation('name') }}</h5>
                                        @php
                                            $sugg_tags = [];
                                            if($suggestion->choice_options) {
                                                foreach(json_decode($suggestion->choice_options) as $option) {
                                                    $attr = \App\Models\Attribute::find($option->attribute_id);
                                                    if($attr && in_array($attr->getTranslation('name'), ['Fabric', 'Style', 'Material', 'Design'])) {
                                                        $sugg_tags[] = $option->values[0];
                                                    }
                                                }
                                            }
                                        @endphp
                                        @if(!empty($sugg_tags))
                                            <div class="mb-1">
                                                @foreach($sugg_tags as $s_tag)
                                                    <span style="font-size: 9px; color: #999; letter-spacing: 0.5px; text-transform: uppercase;">{{ $s_tag }}</span>
                                                    @if(!$loop->last) <span class="mx-1 opacity-50">|</span> @endif
                                                @endforeach
                                            </div>
                                        @endif
                                        <div class="fs-13 fw-700 text-primary">{{ single_price($suggestion->unit_price) }}</div>
                                    </div>
                                    <div class="ml-2">
                                        <button type="button" class="btn btn-soft-primary btn-icon btn-circle btn-sm" 
                                                onclick="quickAddToCart({{ $suggestion->id }}, this)" title="{{ translate('Quick Add') }}">
                        ...
                        <small class="text-muted d-block mt-2 italic">* {{ translate('Based on your cart\'s style DNA') }}</small>
                    </div>
                @endif
            @endif
            ...
            <div class="mt-4">
                <a href="{{ route('checkout.shipping_info') }}" class="btn btn-primary btn-block fs-14 fw-700 rounded-0 px-4">
