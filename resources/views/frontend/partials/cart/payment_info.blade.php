...
<div>
    ...
    <div class="row gutters-10">
        @foreach (get_activate_payment_methods() as $payment_method)
            <div class="col-xl-4 col-md-6">
                <label class="aiz-megabox d-block mb-3">
                    ...
                    <span class="d-flex align-items-center justify-content-between aiz-megabox-elem rounded-0 p-3 hov-shadow-md has-transition">
                        <span class="d-block fw-600 fs-14">{{ ucfirst(translate($payment_method->name)) }}</span>
                        <span class="rounded-1 h-40px w-60px d-flex align-items-center justify-content-end overflow-hidden">
                            <img src="{{ static_asset('assets/img/cards/'.$payment_method->name.'.png') }}"
                            class="mw-100 mh-100" style="object-fit: contain">
                        </span>
                    </span>
                </label>
            </div>
        ...
        @if (get_setting('cmi_payment') == 1)
            <div class="col-xl-4 col-md-6">
                <label class="aiz-megabox d-block mb-3">
                    <input value="cmi" class="online_payment" type="radio" name="payment_option" checked>
                    <span class="d-flex align-items-center justify-content-between aiz-megabox-elem rounded-0 p-3 hov-shadow-md has-transition">
                        <span class="d-block fw-600 fs-14">{{ translate('CMI Payment') }}</span>
                        <span class="rounded-1 h-40px w-60px d-flex align-items-center justify-content-end overflow-hidden">
                            <img src="{{ static_asset('assets/img/cards/cmi.png') }}" class="mw-100 mh-100" style="object-fit: contain" alt="CMI">
                        </span>
                    </span>
                </label>
            </div>
