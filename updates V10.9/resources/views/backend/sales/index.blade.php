...
@section('script')
    <script type="text/javascript">
        ...
        $(document).on('click', '#oc_confirm_btn', function () {
            ...
            const trackingCode = $('#oc_tracking_code').val();
            if (typeof trackingCode !== 'undefined' && trackingCode !== null) {
                calls.push($.ajax({
                    url: "{{ route('orders.update_tracking_code') }}",
                    type: 'POST',
                    data: { _token: CSRF, order_id: ORDER_ID, tracking_code: trackingCode || '' }
                }));
            }
        ...
        $(document).on('click', '#oc_generate_tracking', function () {
            var chars    = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
            var alpha    = '';
            for (var i = 0; i < 4; i++) {
                alpha += chars.charAt(Math.floor(Math.random() * chars.length));
            }
            var timePart = Date.now().toString().slice(-5);
            var tracking = 'TRK-' + alpha + timePart;

            $('#oc_tracking_code').val(tracking);
        });
    </script>
