...
import { AddressDraft, AddressDraftErrors, validateAddressDraft, SavedAddress } from '../../commerce/checkoutState';
...
export interface AccountEditAddressScreenProps {
  onBack: () => void;
  onSaved: () => void;
  onDelete: () => void;
  language?: 'fr' | 'ar';
...
/**
 * Figma node 309:766 — 08-edit-address-form-fr
 *
 * Edit existing address form. Pre-populates from
 * authState.getSelectedAddressForEdit(). On save, updates via
 * authState.updateAddress(). On delete, navigates to the delete
 * confirmation flow.
 */
export const AccountEditAddressScreen: React.FC<AccountEditAddressScreenProps> = ({
  onBack,
  onSaved,
  onDelete,
  language = 'fr',
  ...
  const selectedAddress = authState.getSelectedAddressForEdit();

  const [draft, setDraft] = useState<AddressDraft>(() => {
    if (!selectedAddress) {
      return {
        name: '', phone: '+212 ', city: '', zone: '', addressLine: '',
        apartment: '', postcode: '', deliveryInstructions: '',
        label: 'Maison', isDefault: false,
      };
    }
    return {
      ...
      label: (selectedAddress as any).label || 'Maison',
  ...
  const copy = language === 'ar'
    ? {
        title: 'تعديل العنوان',
        subtitle: 'قم بتعديل معلومات هذا العنوان.',
        ...
        save: 'حفظ التعديلات', deleteLabel: 'حذف هذا العنوان',
      }
    : {
        title: 'Modifier l\'adresse',
        subtitle: 'Modifiez les informations de cette adresse.',
        ...
        label: 'Étiquette de l\'adresse', defaultLabel: 'Adresse par défaut',
        save: 'Enregistrer les modifications', deleteLabel: 'Supprimer cette adresse',
      };
  ...
  const handleSave = () => {
    ...
    authState.updateAddress(selectedAddress.id, {
  ...
  const handleDelete = () => {
    if (selectedAddress) {
      authState.setAddressToDelete(selectedAddress);
      onDelete();
    }
  };
    ...
    <KeyboardAvoidingView style={styles.screen} behavior={Platform.select({ ios: 'padding', default: undefined })}>
      ...
      <View style={[styles.header, isRTL && styles.rowReverse]}>
        ...
        <MayushText variant="pageTitle" color={colors.brand.navy900} style={styles.headerTitle}>
          {language === 'ar' ? 'تعديل العنوان' : 'Modifier l\'adresse'}
        </MayushText>
      ...
      <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        <FormField label={copy.name} icon="user" value={draft.name} error={errors.name}
          placeholder={language === 'ar' ? 'أدخل اسمك الكامل' : 'Nom complet'}
        ...
        <FormField label={copy.instructions} icon="edit-2" value={draft.deliveryInstructions}
          placeholder={language === 'ar' ? 'اختياري' : 'Instructions de livraison (facultatif)'}
        ...
        {/* Delete Link */}
        <TouchableOpacity accessibilityRole="button" onPress={handleDelete} style={styles.deleteLink}>
          <MayushIcon name="trash-2" size={16} color={colors.semantic.error} />
          <MayushText variant="button" color={colors.semantic.error}>{copy.deleteLabel}</MayushText>
        </TouchableOpacity>
      </ScrollView>
    </KeyboardAvoidingView>
  );
...
const styles = StyleSheet.create({
  ...
  deleteLink: {
    marginTop: 14, alignSelf: 'center', flexDirection: 'row', alignItems: 'center', gap: 6,
  },
});
