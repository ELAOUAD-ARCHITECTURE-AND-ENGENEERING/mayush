...
(async () => {
  ...
  const suite = load(path.join(__dirname, '../tests/Step8BOrderDetailTrackingPackagesBehaviorTest.ts'));
  await suite.runStep8BOrderDetailTrackingPackagesBehaviorTests(assert);
  console.log(`STEP 8B BEHAVIOR SUMMARY: ${passes} PASSED, ${failures} FAILED`);
