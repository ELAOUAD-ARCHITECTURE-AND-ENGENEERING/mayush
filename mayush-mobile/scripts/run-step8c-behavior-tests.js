...
(async () => {
  ...
  const suite = load(path.join(__dirname, '../tests/Step8COrderCancellationReviewReorderBehaviorTest.ts'));
  await suite.runStep8COrderCancellationReviewReorderBehaviorTests(assert);
  console.log(`STEP 8C BEHAVIOR SUMMARY: ${passes} PASSED, ${failures} FAILED`);
