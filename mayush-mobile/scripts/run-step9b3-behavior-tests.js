...
(async () => {
  ...
  const testModule = load(path.join(__dirname, '../tests/Step9B3PersistenceAndInteractionIntegrityTest.ts'));
  await testModule.runStep9B3PersistenceAndInteractionIntegrityTests(assert, asyncStorageMock);

  console.log(`STEP 9B.3 BEHAVIOR SUMMARY: ${passes} PASSED, ${failures} FAILED`);
  if (failures > 0 || passes < 25) {
    process.exit(1);
  }
})().catch((err) => {
  console.error('[CRITICAL STEP 9B.3 FAILURE]', err);
  process.exit(1);
});
