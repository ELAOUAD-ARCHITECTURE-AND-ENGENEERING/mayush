...
if (!function_exists('getBaseURL')) {
    function getBaseURL()
    {
        if (empty($_SERVER['HTTP_HOST']) || (isset($_SERVER['SCRIPT_NAME']) && (str_contains($_SERVER['SCRIPT_NAME'], ':') || str_contains($_SERVER['SCRIPT_NAME'], '\\')))) {
            ...
            $path = isset($parsed['path']) && $parsed['path'] !== '/' ? '/' . trim($parsed['path'], '/') . '/' : '/';
...
if (!function_exists('product_restock')) {
    function product_restock($orderDetail)
    {
        ...
        // If no exact variant match, try to get the first stock record (match OrderController::store logic)
        if (!$product_stock) {
...
if (!function_exists('get_promoted_products')) {
    function get_promoted_products($limit = 20)
    {
        return \App\Models\Promotion::where('status', 'approved')
            ->where('start_date', '<=', now())
            ->where('end_date', '>=', now())
            ->with('product', 'product.user', 'product.thumbnail')
            ->take($limit)
            ->get()
            ->pluck('product')
            ->filter(); // Filter out nulls if product deleted
    }
}
...
if (!function_exists('get_image')) {
    ...
    {
        if ($image != null) {
            return uploaded_asset(is_object($image) ? $image->id : $image, $size);
        }
        return static_asset('assets/img/placeholder.jpg');
    }
}
