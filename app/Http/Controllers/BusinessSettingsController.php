...
class BusinessSettingsController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware(['permission:google_recaptcha_configuration'])->only('google_recaptcha', 'cloudflare_turnstile', 'cloudflare_turnstile_update');
    ...
    public function cloudflare_turnstile(Request $request)
    {
        CoreComponentRepository::instantiateShopRepository();
        CoreComponentRepository::initializeCache();
        return view('backend.setup_configurations.google_configuration.cloudflare_turnstile');
    }
    ...
    public function cloudflare_turnstile_update(Request $request)
    {
        foreach ($request->types as $key => $type) {
            $this->overWriteEnvFile($type, $request[$type]);
        }

        $business_settings = BusinessSetting::where('type', 'cloudflare_turnstile')->first();
        if (!$business_settings) {
            $business_settings = new BusinessSetting;
            $business_settings->type = 'cloudflare_turnstile';
        }

        if ($request->has('cloudflare_turnstile')) {
            $business_settings->value = 1;
            $business_settings->save();
        } else {
            $business_settings->value = 0;
