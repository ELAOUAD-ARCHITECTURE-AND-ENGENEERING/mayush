...
class ProductController extends Controller
{
    ...
    {
        $search = null;
        // Check if auction_product and wholesale_product columns exist before querying
        $products = Product::where('user_id', Auth::user()->id)->where('digital', 0);
        
        if (\Illuminate\Support\Facades\Schema::hasColumn('products', 'auction_product')) {
            $products = $products->where('auction_product', 0);
        }
        if (\Illuminate\Support\Facades\Schema::hasColumn('products', 'wholesale_product')) {
    ...
    public function store(ProductRequest $request)
    {
        ...
        ProductTranslation::create($request->only([
            'lang', 'name', 'unit', 'description', 'meta_title', 'meta_description', 'meta_keywords', 'product_id'
        ]));
